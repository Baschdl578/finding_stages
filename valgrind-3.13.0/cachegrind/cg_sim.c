...
static unsigned long long getTimeStampNanos() {
  if (first_timestamp == 1) {
    start = VG_(read_nanosecond_timer)();
    first_timestamp = 0;
    return 0;
  }
	return VG_(read_nanosecond_timer)() - start;
}

static void appendToFile(const char *text) {
  char file[250];
  VG_(sprintf)(file, "/home/schindle/cg_%llu.csv", start);
  if (first_open == 1) {
    fp = VG_(fopen)(file, VKI_O_RDWR|VKI_O_CREAT|VKI_O_APPEND|VKI_O_LARGEFILE,
																			 VKI_S_IRWXU|VKI_S_IRWXG|VKI_S_IRWXO);
    first_open = 0;
  }

  if (!fp) {
    VG_(printf)("CG: Error opening file\n");
    return;
  }

  VG_(fprintf)(fp, "%s", text);
}


static void addIRead(Bool miss, UChar size, Addr addr, HChar* file, HChar* func, Int line) {
  char mi = 'R';
  if (miss) {
    mi = 'M';
  }
  char text[5000];
  VG_(sprintf)(text, "%s;%c;%s;%s;%d;%lu;%u;%llu\n", "Ir", mi, file,
										func, line, addr, size, getTimeStampNanos());
  appendToFile(text);
}

static void addDRead(Addr addr, Bool miss, UChar size, HChar* file, HChar* func, Int line) {
  char mi = 'R';
  if (miss) {
    mi = 'M';
  }
...
static void addDWrite(Addr addr, Bool miss, UChar size, HChar* file, HChar* func, Int line) {
  char mi = 'R';
  if (miss) {
    mi = 'M';
  }
...
__attribute__((always_inline))
static __inline__
void cachesim_I1_doref_Gen(Addr a, UChar size, ULong* m1, ULong *mL, HChar* file, HChar* func, Int line)
{
   if (cachesim_ref_is_miss(&I1, a, size)) {
      (*m1)++;
      if (cachesim_ref_is_miss(&LL, a, size)) {
        addIRead(True, size, a, file, func, line);
         (*mL)++;
       } else {
         addIRead(False, size, a, file, func, line);
       }
   } else {
       addIRead(False, size, a, file, func, line);
   }
...
__attribute__((always_inline))
static __inline__
void cachesim_I1_doref_NoX(Addr a, UChar size, ULong* m1, ULong *mL, HChar* file, HChar* func, Int line)
{
...
__attribute__((always_inline))
static __inline__
void cachesim_D1_doref(Addr a, UChar size, ULong* m1, ULong *mL, HChar* file, HChar* func, Int line, Bool write)
{
   if (cachesim_ref_is_miss(&D1, a, size)) {
      ...
      if (cachesim_ref_is_miss(&LL, a, size)) {
         (*mL)++;
         if (write) {
           addDWrite(a, True, size, file, func, line);
         } else {
           addDRead(a, True, size, file, func, line);
         }
       } else {
         if (write) {
           addDWrite(a, False, size, file, func, line);
         } else {
           addDRead(a, False, size, file, func, line);
         }
       }
   } else {
     if (write) {
